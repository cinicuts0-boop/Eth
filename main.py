...
def dashboard():
    ...
    for coin, data in latest_data.items():
        color = "#FFD700" if coin=="ETH" else "blue"  # 🔶 ETH gold
        cards += f"""
    ...
    <head>
        ...
        <style>
            body {{ font-family: Arial; background:#0f172a; text-align:center; }}
...
def coin_detail(name):
    ...
    history_html = "".join([f"<p>{t['time']} | {t['coin']} {t['type']} @ {t['price']} → {t['result']}</p>"
