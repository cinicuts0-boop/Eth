...
@app.route("/coin/<name>")
def coin_page(name):
    name = name.upper()  # force uppercase to match keys
...
# ===== Common Header =====
def common_header(active="Home"):
    return f"""
    <h1>🚀 Mani Money Mindset 💸</h1>
    <h4>💚 எண்ணம் போல் வாழ்க்கை ❤️</h4>
    <div class="nav">
        <a href="/" {"style='color:cyan'" if active=='Home' else ''}>Home</a> | 
        <a href="/signals" {"style='color:cyan'" if active=='All Charts' else ''}>All Charts</a> | 
        <a href="/rules" {"style='color:cyan'" if active=='Rules' else ''}>Rules</a> | 
        <a href="/tricks" {"style='color:cyan'" if active=='Tricks' else ''}>Tricks</a> | 
        <a href="/alerts" {"style='color:cyan'" if active=='Alerts' else ''}>Alerts</a> | 
        <a href="/thresholds" {"style='color:cyan'" if active=='Thresholds' else ''}>⚙️ Thresholds</a>
    </div>
    """
...
def signals_page():
    ...
    <body>{common_header(active="All Charts")}<h2>📈 Live Signals</h2>{cards}</body></html>
    """
...
def rules_page():
    ...
    """
    return f"<html><body>{common_header(active='Rules')}<h2>📜 Trading Rules</h2>{rules_html}</body></html>"
...
def tricks_page():
    ...
    """
    return f"<html><body>{common_header(active='Tricks')}<h2>💡 Trading Tricks</h2>{tricks_html}</body></html>"

@app.route("/alerts")
def alerts_page():
    history_html = "".join([f"<p>{t['time']} | {t['coin']} | {t['type']} @ {t['price']} → {t['result']}</p>" for t in trade_history[-20:]])
    return f"<html><body>{common_header(active='Alerts')}<h2>📢 Live Alerts</h2>{history_html if history_html else '<p>No alerts yet</p>'}</body></html>"
...
def thresholds_page():
    ...
    if request.method == "POST":
        try:
            ...
            pass
    return f"""
    <html><body>{common_header(active='Thresholds')}
    <div class="box">
    <h3>⚙️ Adjust Thresholds</h3>
    <form method="POST">
        RSI Buy: <input name="rsi_buy" value="{rsi_buy_threshold}"/>
...
def coin_page(name):
    data = latest_data.get(name, {})
    total, wins, loss, pnl, accuracy = calculate_stats()
    history_html = "".join([f"<p>{t['time']} | {t['type']} @ {t['price']} → {t['result']}</p>" for t in trade_history if t["coin"]==name][-10:])
    chart_map = {"ETH":"BINANCE:ETHUSDT","BTC":"BINANCE:BTCUSDT","NIFTY":"NSE:NIFTY",
                 "BANKNIFTY":"NSE:BANKNIFTY","CRUDE":"NYMEX:CL1!"}
    symbol = chart_map.get(name, "")
    return f"""
    <html><body>{common_header()}
    <div class="box"><h2>{name}</h2><p>Price: {data.get('price')}</p><p>RSI: {data.get('rsi')}</p><p>Signal: {data.get('signal')}</p></div>
    <div class="box"><h3>📊 Performance</h3><p>Accuracy: {accuracy}% | PnL: {pnl}</p></div>
    <div class="box"><h3>📈 Chart with Signals</h3><iframe src="https://s.tradingview.com/widgetembed/?symbol={symbol}&interval=5&theme=dark"></iframe></div>
    <div class="box">{history_html if history_html else '<p>No trades</p>'}</div></body></html>
    """
