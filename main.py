...
def nav_bar(active="Home"):
    pages = [("Home","/"),("Alerts","/alerts"),("Rules","/rules"),
             ("Tricks","/tricks"),("Thresholds","/thresholds"),("Signals","/signals")]
    html = '<div class="nav" style="margin:20px 0;">'
    for name, link in pages:
        color = "#22c55e" if name==active else "#FFD700"
        html += f'<a href="{link}" style="color:{color};margin:0 10px;">{name}</a>'
    html += '</div>'
    return html
...
def get_signal_for(symbol, name):
    ...
    try:
        ...
        if rsi_val < rsi_buy_threshold and macd_diff > macd_diff_threshold: signal = "BUY"
        elif rsi_val > rsi_sell_threshold and macd_diff < -macd_diff_threshold: signal = "SELL"
        else: signal = "WAITING"
